    ...
    gameweeks.map((gameweek) => {
        ...
        for (const [teamId, standing] of standingsForWeek) {
            if (selectedEntries.includes(teamId)) {
                const point = { x: gameweek, y: standing.pointsScoreFor };
                const existingPoints = foo[teamId];
                if (existingPoints) {
                    existingPoints.push(point);
                } else {
                    foo[teamId] = [{ x: 0, y: 0 }, point];
                }
            }
        }
    });
...
export const PointsFor = ({ entries, standings, gameweek }: Props) => {
    const [selectedEntries, setSelectedEntries] = useState<number[]>([]);
    const [hovered, setHovered] = useState<number | undefined>(undefined);
    return (
        <>
            <LineGraph
                title="Points For by Gameweek"
                data={[
                    ...buildGraphData(
                        standings,
                        gameweek,
