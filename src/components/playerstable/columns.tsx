  ...
  return players
    ...
    .map((p, i) => {
      ...
      return {
        ...
        draftRankMinusRank:
          p.draft_rank === undefined ? undefined : p.draft_rank - rank,
        draftPickMinusTotalPointsRank:
          p.draftPickNumber === undefined || p.draftPickNumber === "Unknown"
            ? undefined
      ...
      (acc, next) => {
        const games = getGames(next, team.id)
          .map((game) => {
            ...
            const opposition = getTeam(side === "home" ? team_a : team_h);
...
export const buildPlayerRow = (
  ...
  return {
    ...
    xAp90Rank: 0,
    xGIp90: calculatePer90(player.expected_goal_involvements, player.minutes),
    xGIp90Rank: 0,
    ...
    goalInvolvementsMinusExpectedGoalInvolvements:
      player.expected_goal_involvements === undefined
        ? undefined
        : +(goalInvolvements - player.expected_goal_involvements).toFixed(2),
    ...
    draftedBy:
      draftInfo === "Unknown" ? undefined : getEntry(draftInfo?.entry ?? -1),
...
const numberColumn = (
  ...
  title:
    title ??
    key.replaceAll("_", " ").split(" ").map(capitalizeFirstLetter).join(" "),
  abbr,
  description,
  render: (row) =>
...
export const PlayerCol: Column = {
  ...
  render: ({ code, displayName, web_name, position, team }) => {
    const avatarProps = {
      radius: "md",
      src: `https://resources.premierleague.com/premierleague25/photos/players/40x40/${code}.png`,
    } as const;
    return (
      <>
        <User
...
export const OwnerCol: Column = {
  ...
  sort: (a, b) =>
    (a.owner?.manager.name ?? "").localeCompare(b.owner?.manager.name ?? ""),
};
...
export const PositionCol: Column = {
  ...
  abbr: "Pos",
  render: ({ position }) =>
    position === undefined ? "-" : <div>{position.singular_name_short}</div>,
...
export const ElementIdCol = numberColumn("id", {
  title: "FPL Season ID",
  abbr: "ID",
  description:
    "A unique identifier assigned to every player in FPL this season",
});
        ...
        return (
          <>
            {maybeFixtures.map((fixture) => {
              ...
              return (
                <div className="truncate">
                  {opposition.short_name} ({side === "home" ? "H" : "A"})
                </div>
              );
            })}
          </>
...
export const DraftedByCol: Column = {
  ...
  render: ({ draftedBy }) =>
    draftedBy === undefined ? (
      ...
      <ManagerComponent manager={draftedBy.manager} teamName={draftedBy.name} />
    ),
  sort: (a, b) =>
