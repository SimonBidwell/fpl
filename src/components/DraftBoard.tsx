...
const DraftPick = ({ choice }: { choice: Choice }) => {
  ...
  return (
    <Card shadow="sm" style={{ order }} className="text-xs relative">
      <CardBody className="p-2 overflow-hidden h-16">
        ...
        />
        <MoveRight
          className={`bottom-1 left-1 h-4 w-3 absolute ${getRotation(
            round,
            pick,
            12
          )} text-foreground-400`}
...
export const DraftBoard = () => {
  ...
  const draft = (maybeDraft === "Unknown" ? [] : maybeDraft).sort(
    (a, b) => a.index - b.index
  );
      ...
      <div className="grid grid-cols-[repeat(12,_minmax(8rem,_1fr))] gap-1 pt-4 pb-2 align-start text-center">
        {order.map((x, i) => (
          <div className="px-2 flex flex-col gap-1 items-center overflow-hidden">
            ...
            />
            <span className="text-small text-inherit truncate w-full">
              {getEntry(x.entry)?.name}
            </span>
            <span className="text-tiny text-foreground-400 truncate w-full">
              {getEntry(x.entry)?.manager.name}
            </span>
          </div>
        ))}
      </div>
